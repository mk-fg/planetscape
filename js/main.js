...
(function() {
  'use strict';
  var config, conntrace, fs, opts, proj, svg, u, util,
  ...
  (function() {
    var k, p0, scale_factor, _fn, _ref;
  ...
  svg.append('path').datum(d3.geo.graticule()).attr('class', 'graticule').attr('d', proj.path);
  ...
  (function() {
    var k, menu_opts, proj_names, proj_tween, update, v;
    proj_names = (function() {
      var _ref, _results;
      _ref = opts.projections;
      _results = [];
    ...
    proj_tween = function(proj0, proj1) {
      return function(d) {
        var project, t;
        project = function(λ, φ) {
          ...
          λ *= 180 / Math.PI;
          φ *= 180 / Math.PI;
          _ref = [proj0([λ, φ]), proj1([λ, φ])], p0 = _ref[0], p1 = _ref[1];
    ...
    update = function() {
      ...
      _ref = [proj.func, opts.projections[k]()], proj0 = _ref[0], proj.func = _ref[1];
      proj.name = k;
      return svg.selectAll('path').transition().duration(750).attrTween('d', proj_tween(proj0, proj.func));
    };
    menu_opts = d3.select('#projection-menu').style('display', 'block').on('change', update).selectAll('option').data(proj_names);
