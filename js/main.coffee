...
do ->
	...
	[path_conf_order, path_conf_bases] = [[opts.config_path_base], d3.set()]
	while path_conf_order.length
		...
		try path_conf = fs.realpathSync(path_conf)
			...
			continue
		if path_conf in opts.config_path then continue
		...
		if conf.base and not path_conf_bases.has(conf.base)
